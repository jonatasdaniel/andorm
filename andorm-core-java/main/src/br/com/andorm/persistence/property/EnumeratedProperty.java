	...
	public void set(Object in, Object value) {
		if(value != null) {
			if(type == EnumType.Ordinal) {
				...
				Object[] values = (Object[]) invoke(getField().getType().getClass(), method).withoutParams();
				super.set(in, values[ordinal]);
			} else if(type == EnumType.Name) {
				String name = (String) value;
				Method method = in(getField().getType()).returnMethod("valueOf", String.class);
				Object enumValue = invoke(getField().getType().getClass(), method).withParams(name);
